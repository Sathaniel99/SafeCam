...
export const MediaPreview = ({ camera, openLightbox }: MediaPreviewProps) => {
  const isVideo = camera.type === "video";
  const mediaClass =
    "h-full w-auto object-contain pointer-events-none m-auto relative z-2 transform transition-transform duration-500 scale-100 group-hover:scale-110";
  const backgroundClass =
    "h-full w-full object-cover pointer-events-none m-auto absolute top-0 blur-md transform transition-transform duration-500 group-hover:scale-120 z-0";
