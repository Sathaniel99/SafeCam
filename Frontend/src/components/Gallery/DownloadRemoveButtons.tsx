...
export function DownloadRemoveButtons({ camera, URL, onDelete, setModalOpen }: DownloadRemoveButtonsProps) {
    ...
    return (
        <div className="absolute bottom-0 start-0 z-10 flex items-center justify-between w-full p-1">
            ...
            <div className="flex flex-row items-center gap-1">
                {/* Botón de eliminar */}
                <button
                    type="button" onClick={() => { onDelete(camera); setModalOpen(true); }}
                    title={`Eliminar ${mediaType}`} aria-label={`Eliminar archivo ${camera.nombre_fact}`}
                ...
                {/* Botón de descarga */}
                <a
                    href={`${URL}/${camera.url}`} download={camera.nombre_fact} target="_blank" rel="noopener noreferrer"
                    title={`Descargar ${mediaType}`} aria-label={`Descargar ${mediaType} ${camera.nombre_fact}`}
